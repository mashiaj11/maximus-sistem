-- Maximus Admin - reset seguro de dados operacionais
-- Projeto alvo auditado localmente: https://qojanjfnmpkgugkzhypy.supabase.co
--
-- Backup recomendado antes de executar em producao:
...
--     --table=public.delivery_drivers \
--     --data-only --file=maximus-operational-backup.sql
--
-- Preserva:
--   units, admin_settings, products, categories, store_tables,
--   delivery_fee_rules, delivery_neighborhood_rules, Storage, policies e triggers.
...
begin
  ...
  where lower(translate(coalesce(name, ''), 'ãáàâéêíóôõúç', 'aaaaeeiooouc')) in ('joao', 'vitinho')
