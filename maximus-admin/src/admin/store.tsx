...
export function AdminProvider({ children }: { children: ReactNode }) {
  ...
  const [hasLoadedInitialData, setHasLoadedInitialData] = useState<boolean>(!isSupabaseConfigured);
  ...
  const value = useMemo<AdminContextValue>(() => {
    ...
    return {
      ...
      addTable: async () => {
        if (!selectedUnitId) throw new Error("Selecione uma unidade antes de criar mesa.");
        ...
        if (isSupabaseConfigured) {
          if (
            ...
            throw new Error(`Mesa ${String(number).padStart(2, "0")} já existe nesta unidade.`);
          try {
            await insertSupabaseTable(selectedUnitId, number, selectedUnit?.publicAppUrl);
            await loadAdminData();
          } catch (error) {
