...
validateSearch: (search: Record<string, unknown>): MesaSearch => {
  const table = normalizeSearchTable(
    search.table ?? search.mesa ?? search.table_number,
  );
...
function MesaPage() {
  ...
  useEffect(() => {
    ...
    loadPublicMenu(qrUnit, "dine_in")
      .then((data) => {
        const selectedUnit =
          data.units.find((item) => item.slug === qrUnit || item.id === qrUnit) ?? data.units[0];
        setUnitName(selectedUnit?.name ?? qrUnit);
