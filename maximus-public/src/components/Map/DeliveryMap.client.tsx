...
export function LocationPickerMapClient({
  ...
  const validValue = value && isValidCoordinate(value.latitude, value.longitude) ? value : null;
  const validFallback =
    fallback && isValidCoordinate(fallback.latitude, fallback.longitude) ? fallback : null;
  const center = validValue ?? validFallback ?? DEFAULT_CENTER;
  ...
  return (
    ...
    >
      ...
      {validValue && (
        <CircleMarker
          center={[validValue.latitude, validValue.longitude]}
