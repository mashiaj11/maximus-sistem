...
export function CartDrawer({
  ...
  const showMobileBottomCart = count > 0 && (pathname === "/menu" || pathname === "/mesa");
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger asChild>
        <Button
          variant="outline"
          className={`relative ${showMobileBottomCart ? "hidden sm:inline-flex" : ""}`}
          ...
          <div className="w-full space-y-3">
            {isMesaQrFlow && (
              <div className="rounded-xl border border-primary/30 bg-primary/10 p-3 text-xs font-semibold text-primary">
                Mesa {displayMesa} identificada. Escolha comer no local ou levar antes de finalizar.
              </div>
            )}
      ...
      {showMobileBottomCart && (
        <div className="pointer-events-none fixed inset-x-0 bottom-0 z-50 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:hidden">
          <Button
            type="button"
            size="lg"
